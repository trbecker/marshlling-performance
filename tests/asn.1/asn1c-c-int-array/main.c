...
{
	return aper_encode_to_buffer(&asn_DEF_IntArray, NULL, array, encode_buffer, buffer_size);
}

void decode(IntArray_t *array, void *buffer, size_t buffer_size)
{
	aper_decode_complete(NULL, &asn_DEF_IntArray, (void **)&array, buffer, buffer_size);
}

void cycle(int **inarray)
{
	asn_enc_rval_t enc_rval;
	IntArray_t *asnarray = (IntArray_t *)calloc(1, sizeof(IntArray_t));
	assert(asnarray);
	...
	//xer_fprint(stdout, &asn_DEF_IntArray, asnarray);

	free(encode_buffer);
	SEQUENCE_OF_free(&asn_DEF_IntArray, asnarray, 0);
}
